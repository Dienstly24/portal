...
{{-- CSRF-Marke fuer fetch()-Aufrufe aus @push('cspScripts')-Bloecken. --}}
...
/* Markenfarben: resources/css/brand.css (UX-1).
   ...
   (UX-2) - deren Standardmasse SIND die der Beraterwelt, hier steht
   deshalb nur noch die Geometrie dieses Layouts. */
...
.nav-item.active::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--gold-soft);border-radius:0 3px 3px 0;}
...
.nav-badge{margin-left:auto;flex:none;background:var(--gold);color:var(--graphite-deep);border-radius:999px;padding:2px 7px;font-size:11px;font-weight:700;line-height:1.4;}
...
.nav-group-header:focus-visible,.nav-item:focus-visible{outline:2px solid var(--gold-soft);outline-offset:-2px;}
...
.header-avatar{width:36px;height:36px;border-radius:50%;background:var(--graphite);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;cursor:pointer;}
...
.icon-green{background:var(--emerald-soft);color:var(--emerald-deep);}
...
.customer-card:hover{border-color:var(--emerald);background:#F8F9FA;}
