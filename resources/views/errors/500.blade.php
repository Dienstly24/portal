...
<title>{{ $isAr ? 'خطأ تقني' : 'Technischer Fehler' }} – Dienstly24</title>
@include('partials.favicon')
<link rel="stylesheet" href="/fonts/fonts-{{ $isAr ? 'ar' : 'de' }}.css">
<style>
/* Tokens bewusst doppelt - siehe 404.blade.php (kein Vite-Bundle). */
...
.code{font-size:3.4rem;font-weight:800;color:#B3261E;line-height:1;}
...
<body>
<div class="card">
  <div class="code">500<small>{{ $isAr ? 'خطأ تقني' : 'Technischer Fehler' }}</small></div>
  <h1>{{ $isAr ? 'حدث خطأ لدينا – نعمل على إصلاحه.' : 'Da ist etwas schiefgelaufen – wir kümmern uns darum.' }}</h1>
  <p>{{ $isAr
      ? 'يرجى المحاولة مرة أخرى بعد قليل. إذا كان الأمر عاجلاً، تواصلوا معنا مباشرة عبر الهاتف أو واتساب.'
      : 'Bitte versuchen Sie es in wenigen Minuten erneut. Wenn es eilig ist, erreichen Sie uns direkt per Telefon oder WhatsApp.' }}</p>
  <div class="actions">
    <a href="{{ $isAr ? '/ar' : '/' }}" class="btn btn-primary">{{ $isAr ? 'إلى الصفحة الرئيسية' : 'Zur Startseite' }}</a>
    <a href="tel:{{ config('website.phone_e164') }}" class="btn btn-ghost">{{ config('website.phone_display') }}</a>
  </div>
  <div class="links">
