{{--
    Wiederverwendbare Dokument-Vorschau fuer Beraterwelt UND Kundenportal.
    ...
    Erwartete Attribute am ausloesenden Element:
      data-preview-url       Inline-URL des Dokuments (z.B. ...download?view=1 oder ...documents/{id}/view)
    ...
    Nutzt die CSS-Variablen --line/--canvas/--ink-soft, die in beiden Layouts
    (Admin + Portal) definiert sind. Einmal pro Seite einbinden:
      @include('partials.doc_preview')
--}}
@once
{{-- Schnellvorschau (Quick-Look): erscheint beim Ueberfahren, ohne Seitenwechsel. --}}
<div id="docpv-quicklook" style="display:none;position:fixed;z-index:1300;width:min(560px,46vw);height:70vh;background:#fff;border:1px solid var(--line);border-radius:12px;box-shadow:0 18px 50px rgba(0,0,0,.28);overflow:hidden;">
...
<div id="docpv-modal" style="display:none;position:fixed;inset:0;z-index:1400;background:rgba(0,0,0,.62);align-items:center;justify-content:center;padding:24px;">
    <div style="background:#fff;border-radius:14px;width:min(1100px,94vw);height:92vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,.4);">
        <div style="padding:11px 16px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:12px;background:var(--canvas);">
            ...
            <a id="docpv-modal-tab" href="#" target="_blank" rel="noopener" class="btn btn-ghost btn-sm" title="{{ __('Neuer Tab') }}">↗ {{ __('Neuer Tab') }}</a>
            <a id="docpv-modal-download" href="#" class="btn btn-ghost btn-sm" title="{{ __('Herunterladen') }}">⬇ {{ __('Herunterladen') }}</a>
            <button type="button" onclick="docPreview.close()" class="btn btn-ghost btn-sm" title="{{ __('Schließen') }} (Esc)" style="font-size:16px;line-height:1;">✕</button>
        </div>
...
window.docPreview = (function () {
    var TXT = {
        noPreview: @json(__('Für diesen Dateityp ist keine Vorschau möglich – bitte herunterladen.')),
        frameTitle: @json(__('Vorschau')),
    };
    ...
    function buildViewer(url, kind, forModal) {
        ...
        if (kind === 'other') {
            ...
            box.textContent = TXT.noPreview;
            return box;
        }
        var frame = document.createElement('iframe');
        frame.src = url;
        frame.title = TXT.frameTitle;
    ...
    // jeder Mausbewegung aufblitzt). Auf Touch-Geraeten feuert kein mouseover,
    // dort bleibt der Klick -> Modal der Weg.
