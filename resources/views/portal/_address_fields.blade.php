...
<div class="field">
    ...
    <p style="font-size:12px;color:var(--ink-soft);margin-top:4px;">Seit wann bzw. ab wann wohnen Sie unter dieser Anschrift?</p>
</div>
...
<div style="border:1px dashed var(--line);border-radius:10px;padding:14px;margin:14px 0;background:var(--canvas);">
    <div style="font-weight:700;font-size:13.5px;margin-bottom:6px;">📎 Nachweis der Anschrift *</div>
    <div class="field">
        <label style="font-size:12.5px;">Was laden Sie hoch?</label>
        <select name="proof_kind" id="{{ $p }}proof_kind">
            <option value="meldebescheinigung">Meldebescheinigung</option>
            <option value="id_front">Ausweis mit neuer Anschrift (Vorderseite)</option>
            <option value="other">Anderer Nachweis (z. B. Mietvertrag)</option>
        </select>
    </div>
    <div class="field"><input type="file" name="proof" id="{{ $p }}proof" required accept=".pdf,.jpg,.jpeg,.png,.webp"></div>
    <div class="field"><label style="font-size:12.5px;">Rückseite / zweites Dokument (optional)</label><input type="file" name="proof_back" id="{{ $p }}proof_back" accept=".pdf,.jpg,.jpeg,.png,.webp"></div>
    <p style="font-size:11.5px;color:var(--ink-soft);">Erlaubt: PDF oder Foto (JPG, PNG, WEBP), max. 10 MB je Datei. Straße, PLZ und Ort müssen lesbar sein.</p>
</div>
