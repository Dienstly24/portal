    ...
    <div style="border:1px solid var(--line);border-radius:10px;padding:14px 16px;margin-bottom:10px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
            ...
            <div style="text-align:right;">
                @if($req->status === 'open')<span class="badge" style="background:#FEF3C7;color:#92400E;">{{ __($req->statusLabel()) }}</span>
                @elseif($req->status === 'uploaded')<span class="badge" style="background:#E6F1FB;color:#185FA5;">{{ __($req->statusLabel()) }}</span>
                @elseif($req->status === 'approved')<span class="badge" style="background:#D9F4E6;color:#17A65B;">{{ __($req->statusLabel()) }}</span>
                @else<span class="badge" style="background:#FBE9E9;color:#B3261E;">{{ __($req->statusLabel()) }}</span>@endif
            </div>
        </div>
        ...
        <div data-scan-step="choose">
            ...
            <div class="field" style="margin-top:14px;margin-bottom:8px;">
                ...
                <select id="scan-contract">
                    ...
                    <option value="{{ $c->id }}">{{ $c->typeIcon() }} {{ __($c->typeLabel()) }} · {{ $c->insurer }}@if($c->contract_number) ({{ $c->contract_number }})@endif</option>
                    @endforeach
                </select>
            </div>
            @endif
...
<div id="upload-doc-modal" class="d24-modal">
    <div class="d24-modal-box">
        ...
        <div style="font-size:18px;font-weight:700;margin-bottom:6px;">{{ __('Dokument hochladen') }}</div>
        <p style="font-size:12.5px;color:var(--ink-soft);margin-bottom:18px;">{{ __('PDF, Bild (JPG, PNG, HEIC, WEBP), DOC oder XLS – max. 10 MB. Unser Team wird über Ihren Upload informiert.') }}</p>
        <form method="POST" action="{{ route('portal.documents.upload') }}" enctype="multipart/form-data">
            @csrf
            <div class="field"><label>{{ __('Kategorie') }} *</label>
            ...
            @if($contracts->isNotEmpty())
            <div class="field"><label>{{ __('Zu welchem Vertrag gehört das Dokument? (optional)') }}</label>
                <select name="contract_id">
                    <option value="">{{ __('— Keinem Vertrag zuordnen —') }}</option>
                    @foreach($contracts as $c)
                    <option value="{{ $c->id }}">{{ $c->typeIcon() }} {{ __($c->typeLabel()) }} · {{ $c->insurer }}@if($c->contract_number) ({{ $c->contract_number }})@endif</option>
                    @endforeach
                </select>
            ...
            @endif
            <div class="field"><label>{{ __('Datei') }} *</label><input type="file" name="document" required accept=".pdf,.jpg,.jpeg,.png,.webp,.heic,.heif,.gif,.doc,.docx,.xls,.xlsx"></div>
