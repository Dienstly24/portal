...
foreach($customer->timeline as $t) {
    $allEvents->push(['date' => $t->created_at, 'type' => $t->type, 'title' => $t->title, 'desc' => $t->description, 'user' => $t->user?->name]);
}
foreach($customer->contracts as $c) {
    $allEvents->push(['date' => $c->created_at, 'type' => 'contract', 'title' => 'Vertrag hinzugefügt: ' . $c->insurer, 'desc' => $c->contract_number, 'user' => $c->added_by ?? 'System']);
}
foreach($customer->tickets as $t) {
    $allEvents->push(['date' => $t->created_at, 'type' => 'ticket', 'title' => 'Anfrage: ' . $t->subject, 'desc' => ucfirst($t->status), 'user' => $customer->user?->name]);
}
foreach($customer->documents as $d) {
    $allEvents->push(['date' => $d->created_at, 'type' => 'document', 'title' => 'Dokument hochgeladen: ' . $d->file_name, 'desc' => ucfirst($d->category), 'user' => 'System']);
}
$allEvents = $allEvents->sortByDesc('date');
$typeIcons = ['contract'=>'📄','ticket'=>'💬','document'=>'📎','appointment'=>'📅','note'=>'📝','approval'=>'✅','default'=>'📌'];
$typeColors = ['contract'=>'#185FA5','ticket'=>'#B5651D','document'=>'#3B7A57','appointment'=>'#6D28D9','note'=>'#5F5E5A','default'=>'var(--petrol)'];
@endphp
