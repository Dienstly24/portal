...
<div class="card">
    ...
    <div class="field"><label>Anzeigename</label><input type="text" name="name" value="{{ old('name', $a->name ?? '') }}" required aria-label="Anzeigename"></div>
    <div class="field"><label>E-Mail-Adresse</label><input type="email" name="email_address" value="{{ old('email_address', $a->email_address ?? '') }}" required aria-label="E-Mail-Adresse"></div>
    <div class="field">
        <label>Anbieter</label>
        <select name="provider" id="provider-select" required aria-label="Anbieter">
    ...
    <div class="field">
        ...
        <input type="text" name="folders" value="{{ old('folders', $a ? implode(', ', $a->watchedFolders()) : 'INBOX') }}" aria-label="Überwachte Ordner (kommagetrennt)">
    </div>
    <div class="field">
...
<div class="card" id="imap-fields">
    ...
    <div class="field"><label>IMAP-Host</label><input type="text" name="imap_host" value="{{ old('imap_host', $a->imap_host ?? '') }}" placeholder="imap.hostinger.com" aria-label="IMAP-Host"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        <div class="field"><label>Port</label><input type="number" name="imap_port" value="{{ old('imap_port', $a->imap_port ?? 993) }}" aria-label="Port"></div>
        <div class="field">
            <label>Verschlüsselung</label>
            <select name="imap_encryption" aria-label="Verschlüsselung">
    ...
    <div class="field"><label>Benutzername</label><input type="text" name="username" value="{{ old('username', $a->username ?? '') }}" placeholder="entspricht meist der E-Mail-Adresse" aria-label="Benutzername"></div>
    <div class="field">
