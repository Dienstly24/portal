    ...
    <table>
        ...
        <tr>
            <td style="padding:14px 20px;">
                ...
                <div class="muted-xs">{{ $a->email_address }}</div>
                @if($a->last_error)
            ...
            <td class="muted-xs">{{ implode(', ', $a->watchedFolders()) }}</td>
            <td>
                <span class="badge {{ $a->is_active ? 'badge-active' : 'badge-pending' }}">{{ $a->is_active ? 'Aktiv' : 'Inaktiv' }}</span>
            </td>
            <td class="muted-sm">{{ $a->last_synced_at?->lokal()->format('d.m.Y H:i') ?? 'noch nie' }}</td>
            <td class="muted-sm">{{ $a->messages_count }}</td>
            <td style="padding-right:20px;white-space:nowrap;">
                @if($a->isOAuth())
                    @php $connected = !empty(($a->credentials ?? [])['refresh_token']); @endphp
                    <a href="{{ route('admin.email_accounts.oauth', $a->id) }}" class="btn {{ $connected ? 'btn-ghost' : 'btn-emerald' }} btn-sm">{{ $connected ? '🔄 Neu verbinden' : '🔗 Verbinden' }}</a>
                @endif
