...
class AiAssistantController extends Controller
{
    ...
    public function takeOver($customerId)
    {
        ...
        $einstellungen = app(AssistantSettings::class);
        $vorgang = Ticket::where('customer_id', $customer->id)
    ...
    public function storeOffer(Request $request, $customerId)
    {
        ...
        $angebot = AiOffer::updateOrCreate(
            [
            ...
            [
                ...
                'origin' => AiOffer::ORIGIN_EMPLOYEE,
                'created_by' => auth()->id(),
            ]
        );

        app(ConversationJournal::class)->record(
            $conversation,
            AiConversationEvent::EVENT_OFFER_ADDED,
            ['angebot' => $angebot->label, 'produkt' => $angebot->product],
            AiConversationEvent::ACTOR_STAFF,
            auth()->id(),
        );
    ...
    public function destroyOffer($customerId, $offerId)
    {
        ...
        $angebot = AiOffer::where('conversation_id', $conversation->id)
            ->findOrFail($offerId);
     ...
     */
    public function suggestReply($customerId, EmployeeAssistantService $assistant)
    {
        $customer = $this->authorizedCustomer($customerId);
        $conversation = AiConversation::forCustomer($customer->id);
    ...
    public function retry($customerId)
    {
        ...
        AiAssistantLog::where('customer_message_id', $letzte->id)->delete();

        $conversation->resume();
        $conversation->reactivate();

        AnswerCustomerMessageJob::dispatch($letzte->id);
